...
public class WorldListHandler extends MainTM {
	...
	for (World w : Bukkit.getServer().getWorlds()) {
		...
		if (!(MainTM.getInstance().getConfig().getConfigurationSection("worldsList." + loadedWorld).getKeys(false).contains("start"))) { // If not, add it in the list with default parameters
		    MainTM.getInstance().getConfig().set("worldsList." + loadedWorld + ".start", defStart);
		}
		// Check if 'speed' exists
		if (!(MainTM.getInstance().getConfig().getConfigurationSection("worldsList." + loadedWorld).getKeys(false).contains("speed"))) { // If not, add it in the list with default parameters
		    MainTM.getInstance().getConfig().set("worldsList." + loadedWorld + ".speed", defSpeed);
		}
		// Check if 'sleep' exists
		if (!(MainTM.getInstance().getConfig().getConfigurationSection("worldsList." + loadedWorld).getKeys(false).contains("sleep"))) { // If not, add it in the list with default parameters
		    MainTM.getInstance().getConfig().set("worldsList." + loadedWorld + ".sleep", defSleep);
		}
		// Check if 'sync' exists
		if (!(MainTM.getInstance().getConfig().getConfigurationSection("worldsList." + loadedWorld).getKeys(false).contains("sync"))) { // If not, add it in the list with default parameters
		    MainTM.getInstance().getConfig().set("worldsList." + loadedWorld + ".sync", defSync);
		}
	    }
	}
	...
	if (debugMode == true)
	    Bukkit.getServer().getConsoleSender().sendMessage(prefixDebugMode + " " + worldsCfgListDebugMsg + " " + CfgFileHandler.setAnyListFromConfig("worldsList")); // Console debug msg
