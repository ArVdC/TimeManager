...
public class McVersionHandler extends MainTM {
	...
	String completeServerVersion = Bukkit.getVersion().toLowerCase();
	if (debugMode == true)
	    Bukkit.getServer().getConsoleSender().sendMessage(prefixDebugMode + " " + completeVersionDebugMsg + " §e" + completeServerVersion); // Console debug msg
	if (completeServerVersion.contains("(mc: 1.")) { // For usual version syntax
	    ...
	    if (debugMode == true)
		Bukkit.getServer().getConsoleSender().sendMessage(prefixDebugMode + " " + noVersionNumberDebugMsg + " '" + completeServerVersion + "'."); // Console debug msg
	...
	if (mcVersionString.contains(".")) { // But if version is in a " 1.x.x" format, check if a "0" needs to be add before the last number
	    ...
	    if (devMode == true)
		Bukkit.getServer().getConsoleSender().sendMessage(prefixDebugMode + " Needed to adjust the decimal to compare them correctly: §e1." + mcVersionString + "§b = §e1." + firstPart + "." + secondPart + "§b."); // Console dev msg
	...
	try { // Check if value could be parsed as a double
	    ...
	    if (debugMode == true)
		Bukkit.getServer().getConsoleSender().sendMessage(prefixDebugMode + " " + wrongVersionNumberDebugMsg + "\n" + nfe);
	    mcVersion = minRequiredMcVersion;
	    Bukkit.getLogger().warning(prefixTM + " " + unknowVersionMsg); // Console warn msg
	}
	if (debugMode == true)
	    Bukkit.getServer().getConsoleSender().sendMessage(prefixDebugMode + " " + serverMcVersionResultDebugMsg + " §e1." + split3[0] + " §bMC version.");
	return mcVersion;
    }
