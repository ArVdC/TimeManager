...
public class DebugModeHandler extends MainTM {
	...
	if (debugOnOff.equalsIgnoreCase("true")) {
	    debugMode = true;
	    if (debugMode == true)
		Bukkit.getServer().getConsoleSender().sendMessage(prefixDebugMode + " " + enableDebugModeDebugMsg); // Console debug msg
	} else {
	    if (debugMode == true)
		Bukkit.getServer().getConsoleSender().sendMessage(prefixDebugMode + " " + disableDebugModeDebugMsg); // Console debug msg
	    debugMode = false;
	}
    }
