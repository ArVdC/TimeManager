	...
	public static void commandsScheduler() {
		...
		cmdsTask = commandsScheduler.scheduleSyncRepeatingTask(MainTM.getInstance(), new Runnable() {
			...
			public void run() {
				...
				if (MainTM.getInstance().cmdsConf.getString(CF_USECOMMANDS).equalsIgnoreCase("false")) {
					commandsSchedulerIsActive.remove("active");
					stopCmdsScheduler();
					...
					commandsSchedulerIsActive.add("active");
				...
				for (String key : MainTM.getInstance().cmdsConf.getConfigurationSection(CF_COMMANDSLIST).getKeys(false)) {
					...
					// Get the time 
					String eHour = MainTM.getInstance().cmdsConf.getString(CF_COMMANDSLIST + "." + key + "." + CF_HOUR);
					...
					if (!refTimeSrc.contains("UTC") && !refTimeSrc.equalsIgnoreCase("")) {
						...
						currentYear = Integer.parseInt(ValuesConverter.dateFromElapsedDays(cDate, "yyyy"));
						currentMonth = Integer.parseInt(ValuesConverter.dateFromElapsedDays(cDate, "mm"));
						currentDay = Integer.parseInt(ValuesConverter.dateFromElapsedDays(cDate, "dd"));
						// Get the time
						...
						if (speed > 0 && speed <= 1) {
							minutesBeforeEnd = 10;
							ticksBeforeEnd = (long) (167 / speed); // TODO 1.5.0
						} else if (speed > 1 && speed <= MainTM.speedMax) {
							minutesBeforeEnd = 20;
					...
					if (!commandsSchedulerIsActive.contains(key)) {
						if (repeatFreq.equalsIgnoreCase("none")
								&& expectedDateTime.isBefore(currentDateTime)
								&& currentDateTime.isBefore(edgeDateTime)) {
							launchCmds = true;
						} else if (repeatFreq.equalsIgnoreCase("year")) {
						...
						} else if (repeatFreq.equalsIgnoreCase("month")) {
							if ((monthUp && (expectedDay <= currentDay || currentDay <= edgeDay))
									|| (expectedDay <= currentDay && currentDay <= edgeDay)) {
								if ((dayUp && (expectedHour <= currentHour || currentHour <= edgeHour))
										|| (expectedHour <= currentHour && currentHour <= edgeHour)) {
						...
						} else if (repeatFreq.equalsIgnoreCase("day")) {
							if ((dayUp && (expectedHour <= currentHour || currentHour <= edgeHour))
									|| (expectedHour <= currentHour && currentHour <= edgeHour)) {
								if ((hourUp && (expectedMin <= currentMin || currentMin <= edgeMin))
										|| (expectedMin <= currentMin && currentMin <= edgeMin)) {
						...
						} else if (repeatFreq.equalsIgnoreCase("hour")) {
							if ((hourUp && (expectedMin <= currentMin || currentMin <= edgeMin))
									|| (expectedMin <= currentMin && currentMin <= edgeMin)) {
								launchCmds = true;
							}
