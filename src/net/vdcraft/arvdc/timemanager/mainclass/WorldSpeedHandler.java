            	...
            	for(String w : MainTM.getInstance().getConfig().getConfigurationSection("worldsList").getKeys(false)) {
                	...
                	if(speedModifNb > 0.0 && speedModifNb < 1.0 && !(isSpeedRealTime.equals("realTime"))) { // Only treat worlds with decreased timers
                		...
                		// #B. Normal time calculation
	                		...
	                		if(timerMode == true) Bukkit.getServer().getConsoleSender().sendMessage(prefixDebugMode + " Missed ticks for world " + w + " is 0," + missedTicks); // Dev console msg
	                		if(missedTicks > 0) { // But if the decimal is bigger than 0
	                    		...
	                    		if(timerMode == true) Bukkit.getServer().getConsoleSender().sendMessage(prefixDebugMode + " Random roll: " + randomNb); // Dev console msg
	                			if(randomNb <= missedTicks) randomTicks = 1; // If the random number is smaller than or equals the decimal, add 1 tick to the total count
	                		}
	                		if(timerMode == true) Bukkit.getServer().getConsoleSender().sendMessage(prefixDebugMode + " Added " + randomTicks + " random ticks for world" + w); // Dev console msg
