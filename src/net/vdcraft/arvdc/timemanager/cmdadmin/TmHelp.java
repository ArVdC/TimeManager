	...
	public static boolean cmdHelp(CommandSender sender, String[] args) {
		...
		if (argsNb >= 3) {
			if (args[1].equalsIgnoreCase(CMD_SET)) {
				...
				// /tm help set speed
				else if (subCmd.equals(CMD_SET_SPEED)) {
					specificCmdMsg = setSpeedHelpMsg; // Help msg (in case of 2 args)
				}
				// /tm help set speed
				else if (subCmd.contains(CMD_SET_D_SPEED) || subCmd.contains(CMD_SET_N_SPEED)) {
			...
			// Maybe someone could forget the 'set' part, so think of its place
					...
					|| subCmd.contains(CMD_SET_SLEEP) || subCmd.contains(CMD_SET_SPEED)
					|| subCmd.contains(CMD_SET_D_SPEED) || subCmd.contains(CMD_SET_N_SPEED)
					|| subCmd.contains(CMD_SET_START) || subCmd.contains(CMD_SET_SYNC) || subCmd.contains(CMD_SET_TIME)
					|| subCmd.contains(CMD_SET_UPDATE)) {
